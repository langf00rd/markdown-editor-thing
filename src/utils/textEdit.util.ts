...
export function placeBeforeAfterString(symbol: string) {
  ...
  const selectedText = textareaElement.value.substring(
    selectionStart,
    selectionEnd
  ); // Gets selected text string based on the start and end indexes
  const n =
    textareaElement.value.slice(0, selectionStart) +
    symbol +
    selectedText +
    symbol +
    textareaElement.value.slice(selectionEnd); // Places the symbols at start and end and replaces the new string in the textarea
